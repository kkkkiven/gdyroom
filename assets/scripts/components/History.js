cc.Class({
    ...
    onBtnHistoryClicked:function(){
        var self = this;
        console.log("what fuck onhistory")
        cc.vv.userMgr.getHistoryList(function(data){
            ...
            console.log("fuck ===history   data " , data)
    ...
    initRoomHistoryList:function(data){
        for(var i = 0; i < data.length; ++i){
            ...
            for(var j = 0; j < 5; ++j){
                ...
                if(s.userid != 0){
                    node.getChildByName("name" + j).active = true;
                    node.getChildByName("n" + j).active = true;
                    node.getChildByName("s" + j).active = true;
                    node.getChildByName("i" + j).active = true;
                    node.getChildByName("playerinfoline" + j).active = true;
                    node.getChildByName("ID" + j).active = true;
                    node.getChildByName("score" + j).active = true;
                    node.getChildByName("name" + j).getComponent(cc.Label).string = s.name;
                    node.getChildByName("ID" + j).getComponent(cc.Label).string = s.userid;
                    node.getChildByName("score" + j).getComponent(cc.Label).string = s.score-2000;
                }else{
                    node.getChildByName("name" + j).active = false;
                    node.getChildByName("ID" + j).active = false;
                    node.getChildByName("score" + j).active = false;
                    node.getChildByName("n" + j).active = false;
                    node.getChildByName("s" + j).active = false;
                    node.getChildByName("i" + j).active = false;
                    node.getChildByName("playerinfoline" + j).active = false;
                }
            }
        }
