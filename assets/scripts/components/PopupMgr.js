cc.Class({
    ...
    onLoad: function () {
        ...
        this.addBtnHandler("settings/btn_restart");
    ...
    start:function(){
        if(cc.vv.dataMgr.dissoveData){
            this.showDissolveNotice(cc.vv.dataMgr.dissoveData);
        }
    },
    ...
    showDissolveNotice:function(data){
        ...
        for(var i = 0; i < data.states.length; ++i){
            var b = data.states[i];
            var name = cc.vv.dataMgr.seats[i].name;
            if(b){
                this._extraInfo += "\n[已同意] "+ name;
            }
            else{
